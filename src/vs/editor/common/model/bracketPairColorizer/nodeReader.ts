...
export class NodeReader {
	...
	*/
	readLongestNodeAt(offset: Length, predicate: (node: AstNode) => boolean): AstNode | undefined {
		...
		while (true) {
			const curNode = last(this.nextNodes);

			if (!curNode) {
				return undefined;
			}
			const curNodeOffset = last(this.offsets)!;
	...
	private nextNodeAfterCurrent(): void {
		while (true) {
			const currentOffset = last(this.offsets);
			const currentNode = last(this.nextNodes);
			this.nextNodes.pop();
			this.offsets.pop();
			...
			const parent = last(this.nextNodes)!;
